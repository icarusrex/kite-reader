...
import { AssessmentTally, SessionExtras, Step, StepKind, buildCheckout, buildMain, passesAssessment, readyForCheckout, storyFor, storyLevel } from '../engine/session';
import { storyPictureUrl } from '../content/pictures';
import { SessionLog, allPassed, coldCheckDue, currentBasics, currentLevel, failCold, jumpTo, passBasics, passCheckout, passCold, recordAnswer, today } from '../engine/progress';
import { buildBasics, passesBasicsAssessment } from '../engine/basics';
...
type Tally = AssessmentTally;
const emptyTally = (): Tally => ({ answered: 0, correct: 0 });
export type SessionMode = 'guided' | 'practice' | 'explore';

export function Session({ onExit, onParent, extras, level: requestedLevel, basics: requestedBasics, mode = 'guided' }: {
  onExit: (log: SessionLog) => void;
  onParent: () => void;
  ...
  const basicsLesson = useRef(requestedBasics ?? (!requestedLevel && progress.track === 'basics' ? currentBasics(progress) : undefined)).current;
  const startLevel = useRef(requestedLevel ?? currentLevel(progress)).current;
  // Explore assumes prerequisite curriculum knowledge for the selected level, but the sandbox is never persisted.
  const sessionProgress = useRef(mode === 'explore' && requestedLevel ? jumpTo(progress, requestedLevel) : progress).current;
  const [level, setLevel] = useState(startLevel);
  const [queue, setQueue] = useState<Step[]>(() => basicsLesson
    ? buildBasics(sessionProgress, basicsLesson)
    : mode === 'guided' && coldCheckDue(progress, startLevel) ? buildCheckout(startLevel, 'cold') : buildMain(sessionProgress, startLevel, extras));
  const [index, setIndex] = useState(0);
  const [neutral, setNeutral] = useState(false);
  const tallies = useRef<Record<Step['phase'], Tally>>({ main: emptyTally(), checkout: emptyTally(), cold: emptyTally() });
  const requiredTallies = useRef<Record<Step['phase'], Tally>>({ main: emptyTally(), checkout: emptyTally(), cold: emptyTally() });
  const correctItems = useRef(new Set<string>());
  ...
  const completedCurriculum = useRef(false);
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => { if (document.visibilityState === 'visible' && Date.now() - lastInteraction.current < 25000) active.current += 1; }, 1000);
  ...
  const finish = async (endedBy: SessionLog['endedBy'], storyTime = false) => {
    ...
    if (storyTime && mode === 'guided') {
      ...
      setIndex((i) => i + 1); pendingExit.current = log; return;
    }
    onExit(log);
  };

  const wantsStoryTime = () => {
    if (mode !== 'guided' || basicsLesson) return false;
    const n = levelRef.current;
    const guidedSessions = progressRef.current.sessions.filter((s) => !s.practice).length + 1;
    return !!storyFor(n) && guidedSessions % 3 === 0;
  };
  ...
  const phaseComplete = (phase: Step['phase']) => {
    ...
    if (phase === 'cold') {
      if (passesAssessment('cold', t, requiredTallies.current.cold)) {
        ...
        if (allPassed(nextP)) { completedCurriculum.current = true; return appendSteps([{ uid: `done${n}`, kind: 'banner', banner: 'level_done', phase: 'main' }]); }
        const next = currentLevel(nextP); setLevel(next);
    ...
    if (phase === 'main') {
      ...
      if (completedCurriculum.current) return finish('complete');
      if (basicsLesson) {
        if (mode === 'guided' && passesBasicsAssessment(t, requiredTallies.current.main)) { update((p) => passBasics(p, basicsLesson)); void say({ p: 'basics_done' }); }
        return finish('complete');
      }
      if (mode !== 'guided') return finish('complete');
  ...
  const onDone = (step: Step) => (correct: boolean | null) => {
    ...
    if (ended.current) return;
    if (correct !== null) {
      const t = tallies.current[step.phase]; t.answered += 1; if (correct) t.correct += 1;
      if (step.required) { const r = requiredTallies.current[step.phase]; r.answered += 1; if (correct) r.correct += 1; }
      if (correct && step.itemId) correctItems.current.add(step.itemId);
      if (step.itemId && step.itemKind && mode !== 'explore') update((p) => recordAnswer(p, step.itemId!, step.itemKind!, correct));
    ...
    if (correct === false && step.phase === 'main' && !step.reinjected) {
      ...
      const at = Math.min(q.length, index + 4); q = [...q.slice(0, at), copy, ...q.slice(at)]; setQueue(q);
    }
    const key = step.itemId ?? step.word ?? step.g ?? '';
