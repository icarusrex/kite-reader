...
export interface BookAnalysis {
  ...
  ready95: number;
  readyPreteach: number;
  preteach: string[];
  /** Recurring capitalised story names. They are NOT assumed decodable; they are explicit pre-teach candidates. */
  names: string[];
...
export function analyzeBook(book: Book, dict: Set<string>): BookAnalysis {
  ...
  // Names are assistable, not decodable. Count them as beyond until explicitly pre-taught.
  const levels = toks.map((t) => names.includes(t.word.toLowerCase()) ? BEYOND : wordLevel(t.word).level);
  ...
  book.pages.forEach((p, page) => {
    for (const raw of p.text.replace(/[“”"]/g, '').replace(/\s*\n\s*/g, ' ').split(/(?<=[.!?])\s+/)) {
      ...
      // Sentences containing a pre-taught name are mined separately only after runtime knowledge says it is available.
