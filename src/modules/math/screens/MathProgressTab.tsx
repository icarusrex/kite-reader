...
import { mathActiveFrontier, mathMarkKnown, mathResetSkill } from '../engine/state';
...
export function MathProgressTab() {
  ...
  return <>
    ...
    <div className="card"><h2>Curriculum map</h2><p style={{ marginTop: 0 }}>Use <b>Already knows</b> to skip a concept she has clearly got — it opens whatever it was blocking, the same as Reading's jump-to-level. <b>Practise again</b> puts it back in rotation.</p><table><thead><tr><th>Concept</th><th>Domain</th><th>Status</th><th>Evidence</th><th>Needs another look</th><th>Grown-up</th></tr></thead><tbody>
      {MATH_SKILLS.map((skill) => { const state = math.skills[skill.id]; return <tr key={skill.id} className={frontier.has(skill.id) ? 'current-row' : ''}>
        ...
        <td><div className="row" style={{ gap: 6, justifyContent: 'flex-start', flexWrap: 'nowrap' }}>
          {state.phase === 'secure' || state.phase === 'maintenance'
            ...
            : <button className="btn light" onClick={() => updateMath((p) => mathMarkKnown(p, skill.id))}>Already knows</button>}
        </div></td>
      </tr>; })}
    </tbody></table></div>
